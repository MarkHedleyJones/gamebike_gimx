...
void PS4Parser::authenticate(uint16_t length, uint8_t *data) {
  sendAuthRequest(length, data);
}

void PS4Parser::authenticate2(uint16_t length, uint8_t *data) {
  receiveAuthRequest(length, data);
}

void PS4Parser::authenticate3(uint16_t length, uint8_t *data) {
  receiveAuthResponse(length, data);
}
...
void PS4Parser::Parse(uint8_t len, uint8_t *buf) {
  if (len > 1 && buf)  {
    ...
    // else if(buf[0] == 0xf0 || buf[0] == 0xf1 || buf[0] == 0xf2) {
    //   Notify(PSTR("\r\nYOYOYOYOYOYOYOYO!!!!!!!!!!!!!!!!!"), 0x80);
    // }
