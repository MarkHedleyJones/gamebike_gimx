...
uint8_t USB::ctrlReq(uint8_t addr, uint8_t ep, uint8_t bmReqType, uint8_t bRequest, uint8_t wValLo, uint8_t wValHi,
  ...
  if(dataptr != NULL) //data stage, if present
  {
    if(direction) //IN transfer
    {
      ...
      //   if (*(dataptr+i) <= 0xF) Serial.print("0");
...
uint8_t USB::InTransfer(EpInfo *pep, uint16_t nak_limit, uint16_t *nbytesptr, uint8_t* data, uint8_t bInterval /*= 0*/) {
  ...
  *nbytesptr = 0;
  ...
  // USBTRACE2("I got ", pktsize);

  // Serial.print("IN  : ");
  // Serial.print(pep->epAddr, HEX);
  // Serial.print(" : ");
  // for (uint8_t i = 0; i < nbytes; i++) {
  //   // USBTRACE3(" ", data[i], 0x81);
...
uint8_t USB::OutTransfer(EpInfo *pep, uint16_t nak_limit, uint16_t nbytes, uint8_t *data) {
  ...
  // for (uint8_t i = 0; i < nbytes; i++) {
  //   // USBTRACE3(" ", data[i], 0x81);
  //   if (*(data_p+i) <= 0xF) Serial.print("0");
  //   // else Serial.print("");
...
void USB::Task(void) //USB state machine
{
  ...
  // if (usb_task_state == USB_ATTACHED_SUBSTATE_SETTLE){
  //   Serial.print("USB Substate Settle");
  //   Serial.print("\r\n");
  // }
  // else if (usb_task_state == USB_STATE_RUNNING){
  //   Serial.print("USB State Running");
  //   Serial.print("\r\n");
  // }
